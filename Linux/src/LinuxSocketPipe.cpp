...
int8_t LinuxSockPipe::_close() {
  int8_t ret = -1;
  if (0 < _sock_id) {
    close(_sock_id);  // Close the socket.
    if (_sock_path) {
      c3p_log(LOG_LEV_NOTICE, __PRETTY_FUNCTION__, "Closed socket %d (%s)", _sock_id, _sock_path);
      free(_sock_path);
      _sock_path = nullptr;
    }
    else {
      c3p_log(LOG_LEV_NOTICE, __PRETTY_FUNCTION__, "Closed socket %d", _sock_id);
    }
    _sock_id = -1;
    ret = 0;
  }
...
int8_t LinuxSockPipe::console_handler(StringBuilder* text_return, StringBuilder* args) {
  ...
  if (0 == StringBuilder::strcasecmp(cmd, "open")) {
  }
  else {
